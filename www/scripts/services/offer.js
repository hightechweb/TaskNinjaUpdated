...
app.factory('Offer', function(FURL, $firebaseArray, $firebaseObject, $q, Auth, Task, toaster) {
	...
	var Offer = {
		...
		getOffer: function(taskId, offerId) {
			//return $firebaseArray(ref.child('offers').child(taskId).child(offerId)); ORIG
			return ref.child('offers').child(taskId).child(offerId);
		},

		cancelOffer: function(taskId, offerId) {
			//return this.getOffer(taskId, offerId).remove(); ORIG

			toaster.pop('success', "Yourz offer has been cancelled."); // New DT
		...
		notifyRunner: function(taskId, runnerId) {
			...
			Auth.getProfile(runnerId).$loaded().then(function(runner) {
				...
				var notification = $firebaseObject(ref.child('notifications'));
