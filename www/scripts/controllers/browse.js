...
app.controller('BrowseController', function($scope, $routeParams, $firebaseArray, $firebaseObject, toaster, Task, Auth, Comment, Offer) {
	...
	if($routeParams.taskId) {
		...
		console.log(task); //logs Obj task
		console.log($routeParams.taskId); //logs task key id
	...
	function setSelectedTask(task) {
		...
		if($scope.signedIn()) {
			...
			//NEW EDIT DT
			// Check if the selectedTask is cancelled
			$scope.isCancelled = Task.isCancelled;
	...
	$scope.cancelTask = function(taskId) {
		console.log("taskId:" + taskId + " task:" + task + " tasks: ");
		Task.cancelTask(taskId).then(function() { //TypeError: Cannot read property 'then' of undefined | LEO
	...
	$scope.completeTask = function(task) {
		Task.completeTask(task).then(function() { //TypeError: Cannot read property 'then' of undefined | LEO
	...
	$scope.cancelOffer = function(offerId) {
		...
		Offer.cancelOffer($scope.selectedTask.$id, offerId).then(function() {
			toaster.pop('success', "Your offer has been cancelled."); //TypeError: Cannot read property 'then' of undefined | LEO
	...
	$scope.acceptOffer = function(offerId, runnerId) {
		Offer.acceptOffer($scope.selectedTask.$id, offerId, runnerId).then(function() { //TypeError: Cannot read property 'then' of undefined | LEO
